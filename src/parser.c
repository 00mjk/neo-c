...
static BOOL parse_var(unsigned int* node, sParserInfo* info, BOOL readonly)
{
    ...
    if(*info->p != '=') {
        ...
        if(node_type) {
            ...
            add_variable_to_table(info->lv_table, buf, node_type, readonly, NULL, -1, info->mBlockLevel == 0);
        }
    }
    else {
        node_type = NULL;
...
static BOOL parse_variable(unsigned int* node, sNodeType* result_type, char* name, sParserInfo* info, BOOL readonly)
{
    ...
    add_variable_to_table(info->lv_table, name, result_type, readonly, NULL, -1, info->mBlockLevel == 0);
    ...
    if(*info->p == '=' && *(info->p+1) != '=') {
        ...
        *node = sNodeTree_create_define_variable(name, info);
    }
    ...
    if(*info->p == ';') {
        ...
        *node = sNodeTree_create_external_function(fun_name, params, num_params, var_arg, result_type, struct_name, operator_fun, info);
        ...
        for(i=0; i<num_params; i++) {
            ...
            BOOL readonly = FALSE;
            if(!add_variable_to_table(info->lv_table, param->mName, param->mType, readonly, NULL, -1, FALSE))
            {
                return FALSE;
            }
        }
...
static BOOL parse_constructor(unsigned int* node, char* struct_name, sParserInfo* info) 
{
    ...
    if(info->mNumGenerics > 0) {
        ...
        *node = sNodeTree_create_generics_function(fun_name, params, num_params, result_type, MANAGED buf.mBuf, struct_name, sname, sline, info);
        ...
        gNodes[result_node].mSName = sname;
...
static BOOL is_type_name(char* buf, sParserInfo* info)
{
    ...
    return klass || node_type || generics_type_name;
}
    ...
    /// chararacter ///
        ...
        if(strcmp(buf, "lambda") == 0) {
            ...
            *node = sNodeTree_create_null(info);
            ...
            if(!parse_constructor(node, NULL, info)) {
                return FALSE;
            }
        }
