...
sPreprocessorFunction gPreprocessorFunctions[PREPROCESSOR_FUN_MAX];

static void preprocessor_init()
{
    memset(gPreprocessorFunctions, 0, sizeof(sPreprocessorFunction)*PREPROCESSOR_FUN_MAX);
}

static void preprocessor_final()
{
    int i;
    for(i=0; i<PREPROCESSOR_FUN_MAX; i++) {
...
static BOOL add_preprocessor(char* name, MANAGED sBuf* fun_source)
{
    ...
    hash_value = get_hash(name) % PREPROCESSOR_FUN_MAX;
    ...
    while(1) {
        if(func->mName == NULL) {
            ...
            if(func == gPreprocessorFunctions + PREPROCESSOR_FUN_MAX) {
    ...
    while(*p) {
        ...
        else if(!dquort && ((*p == '\n' && *(p+1) == '#') || (p == source->mBuf && *p == '#'))) 
        {
            ...
            if(memcmp(p, "preprocessor", 12) == 0) {
                ...
                sBuf fun_source;
                char name[PREPROCESSOR_FUN_NAME_MAX+1];
                char* p2;
                ...
                if(isalpha(*p)) {
                    while(isalnum(*p) || *p == '_') {
                        *p2++ = *p++;
                        ...
                        if(p2 - name >= PREPROCESSOR_FUN_NAME_MAX) {
                ...
                char name[PREPROCESSOR_FUN_NAME_MAX+1];
                char* p2;
                char* argments[PREPROCESSOR_FUN_ARGUMENTS_NUM+1];
                int num_argments;
                sBuf block;
                sPreprocessorFunction* func;
                int i;
                ...
                while(1) {
                    char* p2;
                    char argment[PREPROCESSOR_FUN_ARGMENT_LENGTH_MAX+1];
                    ...
                    if(*p == '\n') {
                        ...
                        while(1) {
                            if(*p == 0) {
                                ...
                                *p2++ = *p++;
                                ...
                                if(p2 - argment >= PREPROCESSOR_FUN_ARGMENT_LENGTH_MAX)
                                {
                        ...
                        *p2 = 0;
                        ...
                        if(num_argments > PREPROCESSOR_FUN_ARGUMENTS_NUM) {
